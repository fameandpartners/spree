...
module Spree
  class Product < ActiveRecord::Base
    ...
    has_many :variants, -> { where( :is_master => false, :deleted_at => nil ) }, :class_name => 'Spree::Variant'
    scope :variants, -> { order("#{::Spree::Variant.quoted_table_name}.position ASC") }

    has_many :variants_including_master, -> { where(:deleted_at => nil) }, :class_name => 'Spree::Variant'

    has_many :variants_including_master_and_deleted, :class_name => 'Spree::Variant'

    has_many :prices, :through => :variants
    scope :prices, -> { order('spree_variants.position, spree_variants.id, currency') }
    ...
    scope :variant_images, -> { order(:position) }
